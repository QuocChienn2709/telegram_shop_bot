...
def get_binance_rate_live():
    ...
    if _binance_rate_cache["rate"] and (now - _binance_rate_cache["ts"]) < BINANCE_CACHE_TTL:
        return (
            _binance_rate_cache["rate"],
            _binance_rate_cache["source"],
    ...
    _binance_rate_cache = {"rate": manual, "ts": now, "source": "Manual (API loi)", "error": f"{err1} | {err2}"}
...
TEXT_EMOJI_KEYS = {
    ...
    "wallet_title": "Tiêu đề ví", "wallet_balance": "Số dư ví",
    "wallet_topup_success": "Nạp ví thành công", "wallet_not_enough": "Số dư không đủ",
...
def email_confirm_buttons(order_code, uid=None):
    return InlineKeyboardMarkup([
        [button(t(uid, "youtube_email_btn_confirm_send"), callback_data=f"cfmsend_{order_code}", ui_key="send")],
        [button(t(uid, "youtube_email_btn_cancel"), callback_data=f"cfmcancel_{order_code}", ui_key="cancel")],
    ])
        ...
        if email_flow:
            ...
            if key:
                ...
                await safe_edit(query,
                    ...
                    f"<b>{html.escape(t(uid, 'account_info'))}:</b>\n"
                    f"{format_key_display(key, get_user_lang(uid))}", reply_markup=kb)
            else:
...
async def my_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    products_map = {}
    for doc in get_db().products.find({"id": {"$in": product_ids}}, {"id": 1, "name": 1, "emoji_id": 1}):
    ...
    for i, o in enumerate(orders, 1):
        ...
        if o["status"] == "pending":
            kb_rows.append([InlineKeyboardButton(f"{t(uid, 'btn_pay_again')} #{short_code}",
                                                 callback_data=f"backpay_{o['id']}")])
            kb_rows.append([InlineKeyboardButton(f"{t(uid, 'btn_delete_order')} #{short_code}",
                                                 callback_data=f"del_order_{o['id']}")])
        else:
            kb_rows.append([InlineKeyboardButton(f"{t(uid, 'btn_recheck_order')} #{short_code}",
                                                 callback_data=f"recheck_{o['id']}")])
            kb_rows.append([InlineKeyboardButton(f"{t(uid, 'btn_hide_order')} #{short_code}",
...
async def wallet_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (f"{wallet_icon} <b>{html.escape(t(uid, 'wallet_title'))}</b>\n\n"
...
async def topup_check_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if order["status"] == "paid":
        ...
        await safe_edit(query, t_html(uid, "wallet_topup_success", amount=order["amount"], balance=bal)); return
    data = get_payment_status(oc)
    paid = bool(data and data.get("code") == "00" and data.get("data", {}).get("status") == "PAID")
    if paid:
        mark_topup_paid(oc)
...
async def pay_wallet_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bal < order["amount"]:
        ...
        await safe_edit(query,
            ...
            f"Số dư: <code>{bal:,}</code> VND\nCần: <code>{order['amount']:,}</code> VND",
    ...
    try:
        ...
        if state_ram or state_db:
            ...
            if not clean_text.isdigit():
                await safe_reply(update.message, t_html(user.id, "wallet_topup_invalid")); return
...
async def admin_import_products(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ln, line in enumerate(content.splitlines(), 1):
        ...
        try:
            ...
            if len(parts) < 3:
                fail.append((ln, "can >= 3 phan")); continue
            if len(parts) == 3:
...
async def admin_set_product_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await safe_reply(update.message, f"Da dat emoji cho SP <code>{pid}</code>." if ok
...
async def admin_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        text = (f"<b>SP #{p['id']}</b>\nTen: {name_html}\nMo ta: {desc}\n"
                f"Gia: {p['price']:,} VND\nKho: {p['stock']}\nBan: {p['sold']}\n"
                f"Flow: <b>{flow}</b>\nEmoji ID: <code>{p.get('emoji_id') or 'chua dat'}</code>\n\n"
                f"<b>Keys ({len(keys)}):</b>\n")
...
async def admin_setui(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await safe_reply(update.message, f"Da dat emoji {label} cho <code>{key}</code>." if ok
...
async def telegram_webhook(request: Request):
    ...
    try:
        ...
        if msg and msg.get("text"):
            logger.info(f"WEBHOOK RECV: from={msg.get('from', {}).get('id')} text='{msg.get('text')[:50]}'")
...
async def main():
    ...
    # FIX: đăng ký 3 handler RIÊNG BIỆT (không dùng `|` — bug PTB 22.8/Py 3.14)
    app.add_handler(MessageHandler(filters.TEXT, handle_user_text), group=0)
    app.add_handler(MessageHandler(filters.CAPTION, handle_user_text), group=0)
    app.add_handler(MessageHandler(filters.ALL, handle_user_text), group=1)
