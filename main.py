...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ghi nhận user (đã có trong DB khi tạo order, nhưng vẫn đảm bảo)
    ...
    await update.message.reply_text(
        ...
        reply_markup=product_buttons(list_products(per_page=5))
    )
...
async def buy_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Lưu order vào DB (trạng thái pending)
    ...
    if payment_url:
        # Cập nhật context để dùng cho webhook
...
# --- Kiểm tra thanh toán (webhook hoặc manual) ---
async def check_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Gọi API PayOS kiểm tra trạng thái (fallback)
    # Ở đây đơn giản: giả định webhook đã xử lý, nhưng có thể gọi GET /payment-requests/{orderCode}
    # Nếu chưa có webhook, ta tự kiểm tra bằng API
    import requests
    headers = {
...
async def payos_webhook(request):
    """Hàm này sẽ được gắn vào ứng dụng web (Flask/FastAPI)."""
    # Do Telegram Bot không có web server sẵn, ta dùng aiohttp hoặc FastAPI riêng.
    # Ở đây ta tích hợp với aiohttp trong cùng process.
    pass

# --- Admin: Thêm sản phẩm (chỉ admin) ---
async def admin_add_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # /add Tên sản phẩm | Mô tả | Giá | Số lượng | key1,key2,key3
        # Đơn giản hóa: /add Ten 100000 5 key1,key2
        if len(parts) < 5:
...
# --- Webhook server tích hợp (sử dụng aiohttp) ---
    ...
    try:
        ...
        if status == "PAID" and order_code:
            ...
            if order and order["status"] == "pending":
                ...
                if key:
                    update_order_status(order_code, "paid", key)
                    # Gửi thông báo cho user (Telegram)
...
async def main():
    ...
    # Set webhook cho Telegram (dùng cho polling thì không cần, nhưng Render dùng webhook)
    # Ở đây dùng polling vì có web server riêng, nhưng để tối ưu, nên dùng webhook cho Telegram
    # Tuy nhiên trên Render, ta có thể chạy polling + webhook HTTP riêng.
    # Ta sẽ chạy polling (update) và webhook server song song.

    # Chạy bot polling
    bot_task = asyncio.create_task(app.run_polling())

    # Chạy webhook server cho PayOS
    webhook_task = asyncio.create_task(start_webhook_server(app))
