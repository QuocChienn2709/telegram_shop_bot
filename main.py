...
def _fetch_fallback_rate():
    try:
        ...
        if not vnd: return None, "er-api khong co VND"
...
DEFAULT_TEXTS = {
 ...
 "        "admin_email_request_title": "YProductêu cầu thêm vào team",
        "",
admin_email_confirm_btn": "Đã        thêm vào team",
        "admin_email_confirmed "": "[ĐÃ XÁC NHẬN]",
order        "wallet_title": "Ví của bạn", "wallet_balance": "Số dư",
    ...
    "en": {
        ...
        "order_title": "Order", "order_product":_amount": "Amount", "order_pay": "Click to pay",
...
async def safe_send(bot, chat_id, text, **kw):
    try:
        ...
        if _is_entity_error(e):
            return await bot.send_message(chat_id=chat_id, text=_strip_tg_emoji(text),
...
UI_KEYS = {
    ...
    # === NEW: emoji cho nút "Tôi đã chuyển khoản" ===
    "binance_sent": "Nút 'Tôi đã chuyển khoản' (mua SP)",
    "binance_sent_topup": "Nút 'Tôi đã chuyển khoản' (nạp ví)",
}
...
async def validate_custom_emoji(bot, chat_id, emoji_id):
    try:
        ...
        has_entity = any(e.type == "custom_emoji" and
                         getattr(e, "custom_emoji_id", None) == emoji_id
    ...
    try:
        ...
        if tg_user is not None:
            ...
            try:
                ...
                user_info = f"@{chat.username}" if chat.username else (chat.full_name or "?")
            except Exception:
                user_info = "?"
...
async def setlang_callback(update, context):
    ...
    prods = list_products(limit=5, offset=0)
    if not prods:
        await safe_edit(query, t_html(uid, "shop_empty")); return
    ui_icon = ui_emoji_html("shop")
...
async def refresh_products_callback(update, context):
    query = update.callback_query
    uid = query.from_user.id
    if await check_email_block(query, uid): return
    try:
...
async def buy_product(update, context):
    ...
    p = get_product(pid)
    if not p:
        await query.answer(t(uid, "not_found"), show_alert=True); return
    if int(p.get("stock", 0)) <= 0:
        await query.answer(t(uid, "out_of_stock_wait"), show_alert=True); return
...
async def pay_payos_callback(update, context):
    ...
    if not url:
        await safe_edit(query, f"PayOS error: {html.escape(str(err))}", reply_markup=None); return
...
async def pay_binance_callback(update, context):
    ...
    # === FIX: dùng ui_key="binance_sent" cho nút Tôi đã chuyển khoản ===
    kb = InlineKeyboardMarkup([
        [button(t(uid, "binance_sent"), callback_data=f"binance_sent_{order_code}", ui_key="binance_sent")],
...
async def binance_sent_callback(update, context):
    ...
    if order["status"] != "pending":
        await safe_edit(query, t_html(uid, "order_paid"), reply_markup=None); return
        ...
        if email_flow:
            ...
            if es == "awaiting_user_confirm":
                ...
                await safe_edit(query, t_html(uid, "youtube_email_preview", email=html.escape(email)),
                                reply_markup=email_confirm_buttons(order_code, uid=uid)); return
            if es == "pending_admin":
...
async def my_orders(update, context):
    ...
    if not orders:
        kb = InlineKeyboardMarkup([[button(t(uid, "btn_back_shop"), callback_data="back_list", ui_key="back")]])
        await safe_edit(query, t_html(uid, "pending_empty"), reply_markup=kb); return
...
async def delete_pending_order_callback(update, context):
    ...
    try: oc = int(query.data.split("_")[2])
        ...
        await query.answer("Loi du lieu", show_alert=True); return
    order = get_order(oc)
    if not order or order["user_id"] != uid:
        await query.answer("Khong tim thay don", show_alert=True); return
    if order["status"] != "pending":
        await query.answer("Don da xu ly roi", show_alert=True); return
...
async def recheck_cancelled_order_callback(update, context):
    ...
    if order["status"] == "paid":
        ...
        await my_orders(update, context); return
    data = get_payment_status(order_code)
    paid = bool(data and data.get("code") == "00" and data.get("data", {}).get("status") == "PAID")
    if not paid:
        await query.answer(t(uid, "order_recheck_notpaid"), show_alert=True); return
    product = get_product(order["product_id"])
    email_flow = bool(product and product.get("requires_email"))
    if email_flow:
        ...
        key = get_available_key(order["product_id"])
        if not key:
            await query.answer(t(uid, "order_no_key"), show_alert=True); return
...
async def hide_order_callback(update, context):
    ...
    if not order or order["user_id"] != uid:
        await query.answer("Khong tim thay don", show_alert=True); return
    if order["status"] != "cancelled":
        await query.answer("Chi an duoc don da huy", show_alert=True); return
    hide_order(oc, uid)
...
async def handle_topup_amount(update, context):
    uid = update.effective_user.id
    if not context.user_data.get("topup_state"): return False
    context.user_data["topup_state"] = False
    text = (update.message.text or "").strip().replace(".", "").replace(",", "")
    if not text.isdigit():
        await safe_reply(update.message, t_html(uid, "wallet_topup_invalid")); return True
    amount = int(text)
    if amount < 2000:
        await safe_reply(update.message, t_html(uid, "wallet_topup_invalid")); return True
...
async def topup_payos_callback(update, context):
    ...
    if not order or order.get("type") != "topup":
        await safe_edit(query, "Khong tim thay don.", reply_markup=None); return
    url, err = create_payment_link(order_code=oc, amount=order["amount"],
                                   description=f"NAP{oc}",
                                   buyer_name=query.from_user.full_name)
    if not url:
        await safe_edit(query, f"PayOS error: {html.escape(str(err))}", reply_markup=None); return
    kb = InlineKeyboardMarkup([
...
async def topup_binance_callback(update, context):
    ...
    # === FIX: dùng ui_key="binance_sent_topup" cho nút Tôi đã chuyển khoản (nạp ví) ===
    kb = InlineKeyboardMarkup([
        [button(t(uid, "binance_sent"),
                callback_data=f"topup_binance_sent_{oc}",
                ui_key="binance_sent_topup")],
        [button(t(uid, "btn_back"), callback_data="wallet", ui_key="back")],
    ])
    await safe_edit(query, text, reply_markup=kb, disable_web_page_preview=True)
...
async def topup_binance_sent_callback(update, context):
    ...
    await query.answer()
    uid = query.from_user.id
    try: oc = int(query.data.split("_")[3])
    except (ValueError, IndexError): return
    order = get_order(oc)
    if not order or order.get("type") != "topup":
        await safe_edit(query, "Khong tim thay don.", reply_markup=None); return
    if order["status"] != "pending":
        ...
        await safe_edit(query, t_html(uid, "wallet_topup_success",
                                      amount=order["amount"], balance=bal)); return
    await safe_edit(query, t_html(uid, "binance_waiting"), reply_markup=None)
    rate, src, _ = get_binance_rate_live()
    usdt = round(order["amount"] / rate, 2)
    user_info = f"@{query.from_user.username}" if query.from_user.username else (query.from_user.full_name or "?")
...
async def confirm_binance_topup_callback(update, context):
    ...
    if admin_uid not in Config.ADMIN_IDS:
        await query.answer("Khong co quyen", show_alert=True); return
    ...
    if not order or order.get("type") != "topup":
        await query.answer("Khong tim thay don", show_alert=True); return
    if order["status"] != "pending":
        await query.answer(f"Don o trang thai: {order['status']}", show_alert=True); return
    ok = mark_topup_paid(oc)
    if not ok:
        await query.answer("Loi cong vi", show_alert=True); return
...
async def pay_wallet_callback(update, context):
    ...
    order = get_order(order_code)
    if not order or order["status"] != "pending":
        await safe_edit(query, "Đơn không hợp lệ.", reply_markup=None); return
    bal = get_user_balance(uid)
    if bal < order["amount"]:
        kb = InlineKeyboardMarkup([
            [button(t(uid, "btn_topup"), callback_data="topup", ui_key="topup")],
    ...
    if not subtract_balance(uid, order["amount"]):
        await query.answer(t(uid, "wallet_not_enough"), show_alert=True); return
    product = get_product(order["product_id"])
    email_flow = bool(product and product.get("requires_email"))
    if email_flow:
        update_order_status(order_code, "paid", None)
...
async def handle_user_text(update, context):
    ...
    if await handle_topup_amount(update, context): return
    if not EMAIL_RE.match(text_in): return
...
async def confirm_email_callback(update, context):
    ...
    if order.get("email_status") == "confirmed":
        await query.answer("Da xac nhan truoc do", show_alert=True); return
...
async def confirm_binance_callback(update, context):
    ...
    if email_flow:
        ...
        key = get_available_key(order["product_id"])
        if not key:
            await query.answer("Het key! Nap them truoc.", show_alert=True); return
        update_order_status(oc, "paid", key)
...
async def admin_add_product(update, context):
    if update.effective_user.id not in Config.ADMIN_IDS:
        await safe_reply(update.message, "Khong co quyen."); return
    try:
        ...
        if not body:
            await safe_reply(update.message, "Thieu tham so."); return
        if "|" in body:
            name_part, rest = body.split("|", 1)
            name = name_part.strip(); has_desc = True
        else:
            tokens0 = body.split(maxsplit=1)
            if len(tokens0) < 2:
                await safe_reply(update.message, "Thieu gia hoac so luong."); return
            name = tokens0[0].strip(); rest = tokens0[1]; has_desc = False
        if not name:
            await safe_reply(update.message, "Thieu ten san pham."); return
        tokens = rest.strip().split()
        if not tokens:
            await safe_reply(update.message, "Thieu tham so."); return
        if "," in tokens[-1]:
            keys_str = tokens[-1]; tokens = tokens[:-1]
        else:
            keys_str = "-"
        if len(tokens) < 2:
            await safe_reply(update.message, "Thieu gia hoac so luong."); return
        stock_str = tokens[-1]; price_str = tokens[-2]
        description = " ".join(tokens[:-2]).strip() if has_desc else ""
        try:
            price = int(price_str.replace(".", "").replace(",", "").strip())
            if price <= 0: raise ValueError()
        except ValueError:
            await safe_reply(update.message, f"Gia loi: <code>{html.escape(price_str)}</code>"); return
        keys = []
        ...
        if keys:
            ...
            try:
                ...
                await safe_reply(update.message, f"So luong loi: <code>{html.escape(stock_str)}</code>"); return
        emoji_note = ""
        if emoji_id:
...
async def admin_setflow(update, context):
    ...
    if len(parts) < 3:
        await safe_reply(update.message, "Cu phap: <code>/setflow &lt;id&gt; &lt;email|key&gt;</code>"); return
    try: pid = int(parts[1])
    except ValueError:
        await safe_reply(update.message, "ID khong hop le."); return
    flow = parts[2].lower()
    if flow not in ("email", "key"):
        await safe_reply(update.message, "Flow phai la email hoac key."); return
    p = get_product(pid)
    if not p:
        await safe_reply(update.message, f"Khong tim thay SP <code>{pid}</code>."); return
...
async def admin_import_products(update, context):
    ...
    try:
        ...
        await safe_reply(update.message, f"Loi doc: {html.escape(str(e))}"); return
    ok, fail = [], []
    for ln, line in enumerate(content.splitlines(), 1):
        ...
        try:
            ...
            if len(parts) < 3: fail.append((ln, "can >= 3 phan")); continue
            if len(parts) == 3:
...
async def admin_add_key(update, context):
    ...
    try:
        ...
        if len(parts) < 3:
            await safe_reply(update.message, "Cu phap: <code>/addkey &lt;id&gt; &lt;k1,k2,...&gt;</code>"); return
        pid = int(parts[1])
        new_keys = [k.strip() for k in parts[2].split(",") if k.strip()]
        if not new_keys:
            await safe_reply(update.message, "Can >= 1 key."); return
        p = get_product(pid)
        if not p:
            await safe_reply(update.message, f"Khong tim thay SP <code>{pid}</code>."); return
...
async def admin_set_product_emoji(update, context):
    ...
    try:
        ...
        parts = clean.split()
        if len(parts) < 2:
            await safe_reply(update.message, "Cu phap: <code>/setemoji &lt;id&gt; [emoji]</code>"); return
        pid = int(parts[1])
        if not emoji_id:
            await safe_reply(update.message, "Khong tim thay custom emoji."); return
        p = get_product(pid)
        if not p:
            await safe_reply(update.message, f"Khong tim thay SP <code>{pid}</code>."); return
        ok = await validate_custom_emoji(context.bot, update.effective_user.id, emoji_id)
...
async def admin_setdesc(update, context):
    ...
    try:
        parts = update.message.text.split(maxsplit=2)
        if len(parts) < 3:
            await safe_reply(update.message, "Cu phap: <code>/setdesc &lt;id&gt; &lt;mo ta&gt;</code>"); return
...
async def admin_detail(update, context):
    ...
    try:
        parts = update.message.text.split()
        if len(parts) < 2:
            await safe_reply(update.message, "Cu phap: <code>/detail &lt;id&gt;</code>"); return
        pid = int(parts[1])
        p = get_product(pid)
        if not p:
            await safe_reply(update.message, f"Khong tim thay SP <code>{pid}</code>."); return
...
async def admin_list_products(update, context):
    ...
    if total == 0:
        await safe_reply(update.message, "Chua co SP."); return
...
async def admin_list2(update, context):
    ...
    if not products:
        await safe_reply(update.message, "Het."); return
...
async def admin_delete_product(update, context):
    ...
    try:
        parts = update.message.text.split()
        if len(parts) < 2:
            await safe_reply(update.message, "Cu phap: <code>/del &lt;id&gt;</code>"); return
        pid = int(parts[1])
        p = get_product(pid)
        if not p:
            await safe_reply(update.message, f"Khong tim thay SP <code>{pid}</code>."); return
...
async def admin_delete_all(update, context):
    ...
    if len(parts) < 2 or parts[1].lower() != "confirm":
        await safe_reply(update.message, "Xoa TAT CA. Xac nhan: <code>/delall confirm</code>"); return
...
async def admin_confirm_order(update, context):
    ...
    try:
        parts = update.message.text.split()
        if len(parts) < 2:
            await safe_reply(update.message, "Cu phap: <code>/confirm &lt;order_code&gt;</code>"); return
        oc = int(parts[1])
        order = get_order(oc)
        if not order:
            await safe_reply(update.message, "Khong tim thay don."); return
        if order["status"] != "pending":
            await safe_reply(update.message, f"Don o trang thai <b>{order['status']}</b>."); return
        product = get_product(order["product_id"])
        email_flow = bool(product and product.get("requires_email"))
        if email_flow:
            update_order_status(oc, "paid", None)
            ...
            key = get_available_key(order["product_id"])
            if not key:
                await safe_reply(update.message, "Het key. Nap truoc."); return
            update_order_status(oc, "paid", key)
...
async def admin_broadcast(update, context):
    ...
    if len(parts) < 2:
        await safe_reply(update.message, "Cu phap: <code>/broadcast &lt;noi dung&gt;</code>"); return
    msg = parts[1]
...
async def admin_users(update, context):
    ...
    total = count_users()
    if total == 0:
        await safe_reply(update.message, "Chua co user nao."); return
    users = list_users_paginated(limit=per_page, offset=offset)
    if not users:
        await safe_reply(update.message, f"Trang {page} khong co du lieu."); return
    ...
    for i, u in enumerate(users, 1):
        ...
        if username: line += f" @{html.escape(username)}"
        line += f" - {html.escape(full_name)} - Vi: <b>{bal:,}d</b> [{lang}]"
        text += line + "\n"
    nav = []
    if page > 1: nav.append(f"<code>/users {page - 1}</code>")
    if offset + per_page < total: nav.append(f"<code>/users {page + 1}</code>")
...
async def admin_user_detail_cmd(update, context):
    ...
    parts = update.message.text.split()
    if len(parts) < 2:
        await safe_reply(update.message, "Cu phap: <code>/user &lt;user_id&gt;</code>"); return
    try: target_id = int(parts[1])
    except ValueError:
        await safe_reply(update.message, "user_id khong hop le."); return
    u = get_user_detail(target_id)
    if not u:
        await safe_reply(update.message, f"Khong tim thay user <code>{target_id}</code>."); return
    username = u.get("username")
    first = u.get("first_name") or ""
    last = u.get("last_name") or ""
    full_name = f"{first} {last}".strip() or "?"
...
async def admin_topups(update, context):
    ...
    if total == 0:
        await safe_reply(update.message, "Chua co user nao nap vi."); return
    ...
    nav = []
    if page > 1: nav.append(f"<code>/topups {page - 1}</code>")
    if offset + per_page < total: nav.append(f"<code>/topups {page + 1}</code>")
    if nav: text += "\n<i>Dieu huong: " + " | ".join(nav) + "</i>"
    await safe_reply(update.message, text)
...
async def admin_setui(update, context):
    ...
    try:
        ...
        if not setting_key:
            await safe_reply(update.message, f"Key loi: <code>{html.escape(key)}</code>"); return
        if not emoji_id:
            await safe_reply(update.message, "Khong tim thay custom emoji."); return
        ok = await validate_custom_emoji(context.bot, update.effective_user.id, emoji_id)
...
async def admin_setui_force(update, context):
    ...
    try:
        ...
        if len(parts) < 2 or not emoji_id:
            await safe_reply(update.message, "Cu phap: <code>/setui_force &lt;key&gt; [emoji]</code>"); return
        key = parts[1].lower()
        setting_key, _ = _resolve_setui_key(key)
        if not setting_key:
            await safe_reply(update.message, "Key loi."); return
        set_setting(setting_key, emoji_id)
...
async def admin_delui(update, context):
    ...
    parts = update.message.text.split()
    if len(parts) < 2:
        await safe_reply(update.message, "Cu phap: <code>/delui &lt;key&gt;</code>"); return
    key = parts[1].lower()
    setting_key, _ = _resolve_setui_key(key)
    if not setting_key:
        await safe_reply(update.message, "Key loi."); return
...
async def admin_testui(update, context):
    ...
    parts = update.message.text.split()
    if len(parts) < 2:
        await safe_reply(update.message, "Cu phap: <code>/testui &lt;key&gt;</code>"); return
    key = parts[1].lower()
...
async def admin_settext(update, context):
    ...
    try:
        ...
        if lang not in ("vi", "en"):
            await safe_reply(update.message, "Lang phai la vi/en."); return
...
async def admin_deltext(update, context):
    ...
    parts = update.message.text.split()
    if len(parts) < 3:
        await safe_reply(update.message, "Cu phap: <code>/deltext &lt;lang&gt; &lt;key&gt;</code>"); return
...
async def admin_setbinance(update, context):
    ...
    parts = update.message.text.split()
    if len(parts) < 2:
        await safe_reply(update.message, "Cu phap: <code>/setbinance &lt;address&gt; [network]</code>"); return
    addr = parts[1]
    net = parts[2].upper() if len(parts) >= 3 else "TRC20"
    if net not in ("TRC20", "BEP20", "ERC20", "POLYGON"):
        await safe_reply(update.message, "Network khong hop le."); return
...
async def admin_viewbinance(update, context):
    if update.effective_user.id not in Config.ADMIN_IDS: return
    a = get_binance_address(); n = get_binance_network(); manual = get_usdt_rate()
    live, src, err = get_binance_rate_live()
    txt = (f"<b>Binance</b>\n- Address: <code>{html.escape(a) if a else '(chua)'}</code>\n"
           f"- Network: <b>{n}</b>\n- Auto: <b>{'ON' if Config.BINANCE_AUTO_RATE else 'OFF'}</b>\n"
...
async def payos_webhook(request):
    ...
    try:
        ...
        if pc == "00" and oc:
            ...
            order = get_order(oc)
            if not order: return Response(text="OK", status=200)
