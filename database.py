...
def _next_id(name):
    return _get_db().counters.find_one_and_update(
        {"_id": name}, {"$inc": {"seq": 1}}, upsert=True, return_document=True)["seq"]

# PRODUCTS
def add_product(name, description, price, stock, keys_list, emoji_id=None):
    db = _get_db(); nid = _next_id("products")
...
def delete_product(pid): return _get_db().products.delete_one({"id": int(pid)}).deleted_count > 0
def delete_all_products(): return _get_db().products.delete_many({}).deleted_count
def get_product(pid):
...
def count_products(): return _get_db().products.count_documents({"stock": {"$gt": 0}})
...
def count_all_products(): return _get_db().products.count_documents({})

def get_available_key(pid):
    doc = _get_db().products.find_one_and_update(
        {"id": int(pid), "keys": {"$exists": True, "$ne": []}},
        {"$pop": {"keys": -1}, "$inc": {"stock": -1, "sold": 1}}, return_document=False)
...
def _normalize_product(doc):
    if not doc: return None
    d = dict(doc); d["keys"] = json.dumps(d.get("keys") or [])
...
def create_order(order_id, user_id, product_id, quantity, amount, payment_method="payos"):
    ...
    db.users.update_one({"user_id": int(user_id)},
        {"$setOnInsert": {"user_id": int(user_id), "registered_at": datetime.utcnow(),
                          "lang": "vi", "lang_set": False}}, upsert=True)
...
def _normalize_order(doc):
    if not doc: return None
    d = dict(doc); d["id"] = d.get("order_code") or d.get("_id")
    return d

# USERS
def register_user(user_id, username=None, first_name=None, last_name=None):
    _get_db().users.update_one({"user_id": int(user_id)},
...
def set_user_lang(user_id, lang):
    _get_db().users.update_one({"user_id": int(user_id)},
        {"$set": {"lang": lang, "lang_set": True},
         "$setOnInsert": {"user_id": int(user_id), "registered_at": datetime.utcnow()}}, upsert=True)
...
def count_users(): return _get_db().users.count_documents({})
...
def set_setting(key, emoji_id):
    _get_db().settings.update_one({"key": key},
        {"$set": {"emoji_id": emoji_id, "updated_at": datetime.utcnow()}}, upsert=True)

def delete_setting(key): _get_db().settings.delete_one({"key": key})

def get_all_settings():
    return list(_get_db().settings.find({}, {"key": 1, "emoji_id": 1, "_id": 0}))

# TEXTS - có emoji + emoji_char
def get_text(key, default=""):
    """
    Trả về HTML đã render emoji (nếu có).
    Format: <tg-emoji emoji-id="X">EMOJI_CHAR</tg-emoji> value
    Fallback khi bot không gửi được: EMOJI_CHAR value (nhờ safe_reply strip)
    """
    ...
    ec = doc.get("emoji_char") or "🎁"
    if eid:
        return f'<tg-emoji emoji-id="{eid}">{ec}</tg-emoji> {val}'
    if doc.get("emoji_char"):
        return f'{ec} {val}'
    return val

def get_text_raw(key):
    """Trả về dict {value, emoji_id, emoji_char} hoặc None."""
    doc = _get_db().texts.find_one({"key": key})
    if not doc: return None
    return {
        "value": doc.get("value"),
        "emoji_id": doc.get("emoji_id"),
        "emoji_char": doc.get("emoji_char"),
    }

def set_text(key, value, emoji_id=None, emoji_char=None, keep_emoji=True):
    """
    - keep_emoji=True: nếu emoji_id=None thì giữ emoji cũ
    - keep_emoji=False: xóa emoji cũ (set emoji_id=None, emoji_char=None)
    """
    db = _get_db()
    ...
    if emoji_id is not None:
        ...
        if emoji_char is not None:
            upd["emoji_char"] = emoji_char
    elif not keep_emoji:
        db.texts.update_one({"key": key},
            {"$unset": {"emoji_id": "", "emoji_char": ""}, "$set": upd}, upsert=True)
        return
    db.texts.update_one({"key": key}, {"$set": upd}, upsert=True)

def delete_text(key): _get_db().texts.delete_one({"key": key})

def get_all_texts():
    return list(_get_db().texts.find({}, {"key": 1, "value": 1, "emoji_id": 1, "emoji_char": 1, "_id": 0}))
